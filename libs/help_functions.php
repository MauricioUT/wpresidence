...
if (!function_exists('wpestate_check_show_map_marker')):
    ...
    function wpestate_check_show_map_marker() {
       //return false;
        //               'hidden_map'                =>  wpresidence_get_option('wp_estate_hide_marker_pin'),
        global $post;
...
if (!function_exists('wpestate_show_poi_onmap')):
    ...
    function wpestate_show_poi_onmap($where = '') {
        ...
        foreach ($points as $key => $value) {
            $return_value .= '<div class="google_poi' . $where . '" id="' . $key . '"><img src="' . get_theme_file_uri('/css/css-images/poi/' . $key . '_icon.png') . '" class="dashboad-tooltip" alt="' . esc_attr($value) . '" data-placement="right"  data-original-title="' . esc_attr($value) . '" ></div>';
        }
        $return_value .= '</div>';
        return $return_value;
    }
...
if (!function_exists('wpestate_show_price_from_all_details')):
    ...
    function wpestate_show_price_from_all_details($wpestate_currency, $where_currency, $return = 0, $wpestate_prop_all_details = '',$second='no') {
        ...
        if($second=='yes'){
            $price_label = wpestate_return_custom_field($wpestate_prop_all_details, 'property_second_price_label');
            $price_label_before = wpestate_return_custom_field($wpestate_prop_all_details, 'property_label_before_second_price');
            $price = floatval(wpestate_return_custom_field($wpestate_prop_all_details, 'property_second_price'));
        }
...
if (!function_exists('wpestate_price_form_adv_search')):
    ...
    function wpestate_price_form_adv_search($position, $slug, $label) {
        ...
        if ($show_slider_price === 'yes') {
            ...
            $return_string .= '
                    <input type="hidden" id="' . $price_low_id . '"  name="price_low"  value="' . floatval($min_price_slider) . '"/>
                    <input type="hidden" id="' . $price_max_id . '"  name="price_max"  value="' . floatval($max_price_slider) . '"/>
                </div>';
        } else {
            $return_string = '';
            if ($position == 'half') {
...
if (!function_exists('wpestate_show_search_field_with_tabs')):
    ...
    function wpestate_show_search_field_with_tabs($label, $active, $position, $search_field, $action_select_list, $categ_select_list, $select_city_list, $select_area_list, $key, $select_county_state_list, $use_name, $term_id, $adv_search_fields_no, $term_counter) {
        ...
        if ($search_field == 'none') {
            ...
            if (isset($_GET['property_status']) && $_GET['property_status'] != '' && $_GET['property_status'] != 'all' && $active == 'active') {
            ...
            $return_string .= wpestate_build_dropdown_adv_new($appendix, 'statuslist', 'adv_status', $adv_actions_value, $adv_actions_value1, 'property_status', $status_select_list, $active);
            ...
            $return_string .= wpestate_build_dropdown_adv_new($appendix, 'actionslist', 'adv_actions', $adv_actions_value, $adv_actions_value1, 'filter_search_action', $action_select_list, $active);
            ...
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'categlist', 'adv_categ', $adv_categ_value, $adv_categ_value1, 'filter_search_type', $categ_select_list, $active);
            ...
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'adv-search-city', 'advanced_city', $advanced_city_value, $advanced_city_value1, 'advanced_city', $select_city_list, $active);
            ...
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'adv-search-area', 'advanced_area', $advanced_area_value, $advanced_area_value1, 'advanced_area', $select_area_list, $active);
            ...
            $return_string = wpestate_build_dropdown_adv_new($appendix, 'adv-search-countystate', 'county-state', $advanced_county_value, $advanced_county_value1, 'advanced_contystate', $select_county_state_list, $active);
        } else {
        ...
        if ($search_field == 'none') {
            ...
            if ($adv_search_what_key == 'property country' || $search_field == 'property country') {
                ...
                if (!empty($custom_fields)) {
                    while ($i < count($custom_fields)) {
                        ...
                        if (($slug_drop == $adv_search_what_key || $slug_drop == $search_field) && $custom_fields[$i][2] == 'dropdown') {
                            ...
                            $front_name = sanitize_title($adv_search_label[$key]);
                            if ($placeholder != '') {
                                $front_name = wpestate_limit45(sanitize_title($elementor_label)); //is elementor
                            }
                            ...
                            if (isset($_GET[$front_name]) && $_GET[$front_name] != '' && $_GET[$front_name] != 'all') {
                                ...
                                $advanced_drop_value1 = 'all';
                                if ($placeholder != '') {
                                    $advanced_drop_value = $placeholder; // from elementor
                                }
                            }
                            $front_name = wpestate_limit45($front_name);
                            $return_string = wpestate_build_dropdown_adv($appendix, $front_name, $front_name, $advanced_drop_value, $advanced_drop_value1, $front_name, $action_select_list);
                ...
                if ($found_dropdown == 0) {
                    ...
                    ////////////////// apply datepicker if is the case
                    if (isset($adv_search_how[$key]) || $item_field_how != '') {

                        if (isset($adv_search_how[$key]) && ( $adv_search_how[$key] == 'date bigger' || $adv_search_how[$key] == 'date smaller' || $item_field_how == 'date bigger' || $item_field_how == 'date smaller')) {
