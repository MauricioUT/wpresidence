...
if(!function_exists('wpestate_custom_fonts_elements')):
    function wpestate_custom_fonts_elements(){
        ...
        if ($h2_fontfamily != '') {
            $style.=  '.featured_prop_type5 h2,h2,h2 a{font-family:' . $h2_fontfamily .';}';
        }
        if ($h2_fontsize != '') {
        ...
        if ($menu_fontweight != '') {
            $style.= '#access ul ul a,.submit_listing,.header_phone,#access a,#user_menu_u{font-weight:' . $menu_fontweight .';}';
        }
...
if( !function_exists('wpestate_general_design_elements') ):
    function wpestate_general_design_elements(){
        ...
        if($wp_estate_logo_max_height!=''){
            $style.='.logo img, .header_wrapper.header_type4 .logo img{
...
function wpestate_share_unit_desing($prop_id,$is_single=0){
    ...
    <div class="share_unit">
        ...
        <a href="<?php print esc_html($protocol);?>://api.whatsapp.com/send?text=<?php echo urlencode( $title.' '. esc_url( $link )); ?>" class="social_whatsup" rel="noreferrer" target="_blank"><?php echo esc_html($whatsup_label);?></a>
