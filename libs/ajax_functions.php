...
function wpestate_modal_social_share($postid){
    ...
    $return ='<div class="prop_social">
                ...
                <a href="https://twitter.com/intent/tweet?text='.urlencode(get_the_title($postid) .' '.  esc_url( get_permalink($postid) )).'" rel="noreferrer"  class="share_tweet" target="_blank"><i class="fab fa-twitter"></i></a>';
                ...
                $return.='<a href="https://api.whatsapp.com/send?text='.urlencode( get_the_title($postid).' '. esc_url( get_permalink() )).'" class="social_email" rel="noreferrer"  ><i class="fab fa-whatsapp" aria-hidden="true"></i></a>
 ...
 * 
 * */
function wpestate_show_license_form($simple=''){
    ...
    $licese=get_option('envato_purchase_code_7896392','');


    if($theme_activated!='is_active' || ( $licese=='' && $current_screen->id=='appearance_page_one-click-demo-import' )  ){
...
function wpestate_secondary_lic(){
    ...
    if($theme_activated!='is_active'){
        ...
        if( intval($theme_active_time) +24*60*60 < time() ){
            exit();
        }
...
if( !function_exists('wpestate_load_recent_items_sh') ):
    function wpestate_load_recent_items_sh(){
       // check_ajax_referer( 'wpestate_ajax_filtering', 'security' );
        ...
        echo wpestate_display_listings_sh($page,  $shortcode_arguments);
...
function wpestate_display_listings_sh($page,$shortcode_arguments){
    ...
    $align_class='';
    if($shortcode_arguments['align']   =='horizontal'){
        $align="col-md-12";
    ...
    function wpestate_ajax_filter_listings_search(){
        ...
        if( $prop_selection->have_posts() ){
                ...
                while ($prop_selection->have_posts()): $prop_selection->the_post();
                      include( locate_template('templates/property_unit'.$property_card_type_string.'.php') );
                endwhile;
            wpestate_pagination_ajax($prop_selection->max_num_pages, 2,$paged,'pagination_ajax_search');
        }else{
            print '<span class="no_results">'. esc_html__("We didn't find any results","wpresidence").'</>';
        }
...
function wpestate_ajax_register_user(){
        ...
        if($enable_user_pass_status=='yes' ){
            ...
            if ($user_pass=='' || $user_pass_retype=='' ){
                echo json_encode(
                    array('register'=>false, 
                           'message'=>esc_html__('One of the password field is empty!','wpresidence'))
                    );
                exit();
            }
...
if( !function_exists('wpestate_register_as_user') ):
    function  wpestate_register_as_user($user_name,$user_id,$new_user_type=0,$first_name='',$last_name=''){
        ...
        if($admin_submission_user_role==''){
            $admin_submission_user_role=array();
        }
...
if( !function_exists('wpestate_ajax_filter_listings') ):
  ...
  function wpestate_ajax_filter_listings(){
       // check_ajax_referer( 'wpestate_ajax_filtering', 'security' );
        ...
        $property_card_type_string      =   wpestate_return_property_card_type($_POST);
        ...
        $is_shortcode=0;
        if(isset($_POST['ishortcode']) &&  $align_class!='the_list_view' ){
            global $is_shortcode;
            $is_shortcode=intval($_POST['ishortcode']);
            $row_number_col       =  wpestate_shortocode_return_column(intval($_POST['rownumber']));
            $wpestate_no_listins_per_row=intval($_POST['rownumber']);
        ...
        if( $prop_selection->have_posts() ){
            while ($prop_selection->have_posts()): $prop_selection->the_post();
                ...
                include( locate_template('templates/property_unit'.$property_card_type_string.'.php') );
            endwhile;
            wpestate_pagination_ajax_newver($prop_selection->max_num_pages, 2,$paged,'pagination_ajax',$order);
        }else{
            print '<span class="no_results">'. esc_html__("We didn't find any results","wpresidence").'</>';
        }
  ...
  function wpestate_ajax_filter_listings_old(){
       // check_ajax_referer( 'wpestate_ajax_filtering', 'security' );
        wp_suspend_cache_addition(true);
