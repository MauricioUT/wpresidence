    ...
    if( isset($post->ID) && !wpestate_half_map_conditions ($post->ID) ){
        ...
        if( is_singular ('post') ){
            ...
            if($header_type==0){ // global
                $header_type_single_post= esc_html ( wpresidence_get_option('wp_estate_header_type_blog_post','') ); 
                
                if($header_type_single_post==0){
                    $header_type=1; //none
                }else if($header_type_single_post==1){
                    $header_type=2; //image
                    $custom_image =  wpresidence_get_option('wp_estate_header_blog_post_image','url');
                }else if($header_type_single_post==2){
                    $header_type=3; //theme slider
                }else if($header_type_single_post==3){
                      $header_type=4; //rev slider
                      $rev_slider = wpresidence_get_option('wp_estate_header_blog_post_revolution_slider','');
                }else if($header_type_single_post==4){
...
<div class="header_media  <?php echo esc_attr($mobile_header_media_sticky); ?> header_mediatype_<?php echo esc_attr($header_type); ?> with_search_<?php echo esc_html($adv_search_type).' '.esc_attr($args['elementor_class']); ?>">
    <?php
